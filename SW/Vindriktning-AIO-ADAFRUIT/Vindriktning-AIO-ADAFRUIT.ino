...
void setup()
{
  ...
  do
  {
    screen.setFont(WELCOME_FONT);
    String welcome_text = String(WELCOME_MESSAGE);
    int wlkm_width = screen.getUTF8Width(welcome_text.c_str());
    screen.drawUTF8((132 - wlkm_width) / 2, 28, welcome_text.c_str());
...
{	
  screen.sendF("cac", 0x0db, v << 4, 0xaf);
}
...
void display_data()
{
  ...
  do
  {
    screen.setFont(u8g2_font_helvB18_tf);

    String co2_val = co2 != -1 ? String(co2) : String("ERR");
    int co2_width = screen.getStrWidth(co2_val.c_str());
    screen.drawStr(64 - co2_width, 21, co2_val.c_str());

    String pm_val = pm2_5 != 65535 ? String(pm2_5) : String("ERR");
    int pm_width = screen.getStrWidth(pm_val.c_str());
    screen.drawStr(128 - pm_width, 21, pm_val.c_str());

    screen.setFont(u8g2_font_helvB08_tf);
    screen.drawStr(44, 30, "ppm"); // width 20
    screen.drawUTF8(100, 30, "µg/m³"); // width 32
  } while (screen.nextPage());
}
#endif
